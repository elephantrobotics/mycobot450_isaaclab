...
All the environments are registered in the `mycobot450_isaaclab` extension. They start
...
with contextlib.suppress(ImportError):
    import mycobot450_isaaclab.tasks_experimental  # noqa: F401
...
def _format_presets(preset_map: dict | None) -> str:
    ...
    Args:
        preset_map: Mapping of preset selector type to sorted preset name lists, or
            ``None`` when the env cfg could not be loaded.
...
def main():
    """Print all environments registered in `mycobot450_isaaclab` extension."""
    # Collect matching task specs first so we can enumerate presets in one pass.
    task_specs = [
        spec
        for spec in gym.registry.values()
        if (
            ("Mycobot-Pro450" in spec.id or spec.id == "mycobot450_isaaclab")
